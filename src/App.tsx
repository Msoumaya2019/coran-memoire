...
type Reader={range:Range;sessionId?:string;revisionId?:string;initialLanguage?:'ar'|'fr'};
...
function AppContent(){
  ...
  useEffect(()=>{const subscription=BackHandler.addEventListener('hardwareBackPress',()=>{
    ...
    return false;
  });return()=>subscription.remove();},[reader,page,socialView,tab,wizard]);
...
function Home({state,prog,stat,todaySessions,due,finishEstimate,openReader,setTab,unreadCount,openMessages}:{state:AppState;prog:ReturnType<typeof progress>;stat:ReturnType<typeof stats>;todaySessions:Session[];due:AppState['revisions'];finishEstimate?:string;openReader:(r:Reader)=>void;setTab:(tab:Tab)=>void;unreadCount:number;openMessages:()=>void}){
  ...
  return <>
    ...
    <View style={{flexDirection:'row',justifyContent:'space-between',gap:4,marginBottom:19}}>{[
      ...
      shortcut('Répétition','↻',()=>due[0]?openReader({range:due[0],revisionId:due[0].id}):resume()),
...
function ReaderScreen({reader,page,setPage,masked,setMasked,revealed,setRevealed,onClose,state,update,fullscreen,setFullscreen}:{reader:Reader;page:number;setPage:(n:number)=>void;masked:boolean;setMasked:(v:boolean)=>void;revealed:number|null;setRevealed:(n:number|null)=>void;onClose:()=>void;state:AppState;update:(s:AppState)=>void;fullscreen:boolean;setFullscreen:(value:boolean)=>void}){
  ...
  const chooseMushaf=()=>Alert.alert('Affichage du Coran','Choisis une présentation.',[
    ...
    {text:'Moushaf Tajweed (en préparation)',onPress:()=>Alert.alert('Source à vérifier','Les pages couleur et les coordonnées des versets de cette même édition ne sont pas encore utilisables avec les droits nécessaires.')},
    {text:'Annuler',style:'cancel'},
  ]);
