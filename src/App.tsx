...
import { Alert, AppState as DeviceAppState, BackHandler, Keyboard, Linking, PanResponder, Platform, Pressable, ScrollView, StatusBar, Switch, Text, View, useWindowDimensions } from 'react-native';
import { SafeAreaProvider, SafeAreaView } from 'react-native-safe-area-context';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { applyTheme, Button, Card, CheckChoice, Choice, colors, Field, Label, Title } from './ui/theme';
import { AppState, beginnerPaces, completeSession, dateKey, dayOf, defaultState, generateProgram, goalFromPreset, goalIds, GoalPreset, goalIsAlreadyKnown, goalPresetLabels, gradeRevision, intensivePaces, isRangeKnown, LearningDirection, markKnowledge, paceLabels, pacePresets, PacePreset, partialKnownRanges, postponeSession, progress, reconcileState, resetAllProgress, seedInitialRevisions, Session, stats, todayLocal, toggleKnownRange, touch, validGoal, weekdays } from './core/program';
...
function AppContent(){
  ...
  const closeReader=()=>{setReader(null);setReaderFullscreen(false);};
  ...
  return <SafeAreaView edges={readerFullscreen?['bottom']:['top','bottom']} style={{flex:1,backgroundColor:colors.cream}} {...(reader||socialView==='friends'?{}:edgeBack.panHandlers)}><StatusBar hidden={readerFullscreen} />
    {reader?<ReaderScreen reader={reader} page={page} setPage={setPage} masked={masked} setMasked={setMasked} revealed={revealed} setRevealed={setRevealed} onClose={closeReader} state={state} update={update} fullscreen={readerFullscreen} setFullscreen={setReaderFullscreen} />:
...
function Home({state,prog,stat,todaySessions,due,finishEstimate,openReader,unreadCount,openMessages}:{state:AppState;prog:ReturnType<typeof progress>;stat:ReturnType<typeof stats>;todaySessions:Session[];due:AppState['revisions'];finishEstimate?:string;openReader:(r:Reader)=>void;unreadCount:number;openMessages:()=>void}){
  return <>
    <View style={{paddingTop:12,paddingBottom:20,flexDirection:'row',alignItems:'center',gap:10}}><View style={{flex:1}}><Label style={{color:colors.muted}}>Bonjour{state.profile?.firstName?` ${state.profile.firstName}`:''} et bienvenue dans ton programme de mémorisation.</Label><Title>Ton chemin, jour après jour</Title></View><Pressable accessibilityLabel={`Messages, ${unreadCount} non lus`} onPress={openMessages} style={{padding:10,borderRadius:14,backgroundColor:colors.soft}}><Text style={{fontSize:24,color:colors.green}}>✉</Text>{unreadCount>0?<Text style={{color:colors.red,fontWeight:'700',textAlign:'center'}}>{unreadCount>99?'99+':unreadCount}</Text>:null}</Pressable></View>
...
function ProfileScreen({state,update,account,setAccount,setNotice,openKnowledge,openGoal,openFriends,openAdmin,admin,passwordRecovery,setPasswordRecovery,onPasswordReady,onReset}:{state:AppState;update:(s:AppState)=>void;account:string|null;setAccount:(v:string|null)=>void;setNotice:(v:string)=>void;openKnowledge:()=>void;openGoal:()=>void;openFriends:()=>void;openAdmin:()=>void;admin:boolean;passwordRecovery:boolean;setPasswordRecovery:(v:boolean)=>void;onPasswordReady:()=>void;onReset:()=>Promise<void>}){
  ...
  return <><View style={{paddingTop:12,paddingBottom:14}}><Title>Mon profil</Title><Label style={{color:colors.muted}}>Tes préférences et tes données</Label></View>
    ...
    <Card><Label style={{fontWeight:'700'}}>Affichage du Coran</Label><Label style={{color:colors.muted,fontSize:13,marginVertical:8}}>Choisis la présentation arabe des pages.</Label><Choice label="Moushaf traditionnel de Médine" selected={(state.reader?.mushaf??'traditional')==='traditional'} onPress={()=>update(touch({...state,reader:{mushaf:'traditional',followAudio:state.reader?.followAudio!==false}}))} /><Choice label="Moushaf avec Tajweed en couleur" selected={state.reader?.mushaf==='tajweed'} onPress={()=>update(touch({...state,reader:{mushaf:'tajweed',followAudio:state.reader?.followAudio!==false}}))} /><View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:12,marginTop:8}}><Label style={{flex:1}}>Suivre automatiquement la récitation sur la page suivante</Label><Switch value={state.reader?.followAudio!==false} onValueChange={value=>update(touch({...state,reader:{mushaf:state.reader?.mushaf??'traditional',followAudio:value}}))} trackColor={{false:colors.line,true:colors.green2}} thumbColor={colors.paper} /></View></Card>
...
function ReaderScreen({reader,page,setPage,masked,setMasked,revealed,setRevealed,onClose,state,update,fullscreen,setFullscreen}:{reader:Reader;page:number;setPage:(n:number)=>void;masked:boolean;setMasked:(v:boolean)=>void;revealed:number|null;setRevealed:(n:number|null)=>void;onClose:()=>void;state:AppState;update:(s:AppState)=>void;fullscreen:boolean;setFullscreen:(value:boolean)=>void}){
  ...
  const [language,setLanguage]=useState<'ar'|'fr'>('ar'),[selectedVerse,setSelectedVerse]=useState<number|null>(null),[translationOptions,setTranslationOptions]=useState(false),[audioCommand,setAudioCommand]=useState<AudioCommand|null>(null),[commandSerial,setCommandSerial]=useState(0),[commandsVisible,setCommandsVisible]=useState(true);
  ...
  return <View style={{flex:1}}>
    ...
    <ScrollView ref={readerScroll} style={{flex:1}} contentContainerStyle={{alignItems:'center',paddingBottom:Math.max(100,fullscreen?35:100)}}>
      ...
      <View {...swipe.panHandlers} style={{width:imageWidth}}><MushafPage page={page} width={imageWidth} height={imageHeight} mode={state.reader?.mushaf??'traditional'} language={language} playingVerseId={playingVerseId} sessionRange={reader.range} showSession={!!(reader.sessionId||reader.revisionId)} masked={masked} revealed={revealed} onVerseLongPress={id=>{setSelectedVerse(id);revealCommands();}} onBlankLongPress={()=>{setTranslationOptions(true);revealCommands();}} onTap={revealCommands} /></View>
