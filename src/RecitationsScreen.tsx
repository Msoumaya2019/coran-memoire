...
import {deleteLocalRecitation,deleteMyRecitation,GeneralFeedback,listCorrections,listGeneralFeedback,listRemoteRecitations,localRecitations,LocalRecitation,RemoteRecitation,signedAudioUrl,syncPendingRecitations,VerseCorrection} from './services/recitations';
...
export function RecitationsScreen({onClose,initialRecitationId}:{onClose:()=>void;initialRecitationId?:string|null}){
  ...
  useEffect(()=>{const item=remote.find(row=>row.id===initialRecitationId);if(item&&selected?.id!==item.id)open(item).catch(error=>setMessage(String(error)));},[remote,initialRecitationId]);
  ...
  return <ScrollView contentContainerStyle={{padding:18,paddingBottom:50}}><Pressable onPress={onClose}><Label style={{fontSize:20}}>‹ Retour</Label></Pressable><Title>Mes récitations</Title><Label style={{color:colors.muted,marginTop:5,marginBottom:14}}>Enregistrements sauvegardés sur ce téléphone et synchronisés avec ton compte.</Label>
    ...
    {all.map(item=>{const own=local.find(row=>row.id===item.id);const correctionCount=selected?.id===item.id?corrections.length:0;return <Card key={item.id}><Pressable onPress={()=>open(item)}><Label style={{fontWeight:'700'}}>{reference({start:item.start_verse_id,end:item.end_verse_id})}</Label><Label style={{color:colors.muted,fontSize:12,marginTop:5}}>{new Date(item.created_at).toLocaleString('fr-FR')} · {fmt(item.duration_ms)} · {own?.syncStatus==='synced'||!own?'Synchronisé':own.syncStatus==='uploading'?'En cours d’envoi':own.syncStatus==='failed'?'Échec de synchronisation':'En attente'}</Label></Pressable>
      ...
      {sharing===item.id&&<><Label style={{fontSize:12,color:colors.muted}}>Choisis un ami. L’envoi sera confirmé avant le partage.</Label>{friends.map(friend=><Button key={friend.id} small secondary onPress={()=>Alert.alert('Partager cette récitation ?',`Seul ${friend.other?.display_name??'cet ami'} pourra écouter ${reference({start:item.start_verse_id,end:item.end_verse_id})} tant que vous restez amis.`,[{text:'Annuler',style:'cancel'},{text:'Partager',onPress:()=>shareRecitation(friend.id,item.id,`Récitation vocale · ${reference({start:item.start_verse_id,end:item.end_verse_id})}`).then(()=>{setSharing(null);setMessage('Récitation partagée dans votre conversation.');}).catch(error=>setMessage(String(error)))}])}>{friend.other?.display_name??'Ami'}</Button>)}{!friends.length&&<Label>Aucun ami accepté pour le moment.</Label>}</>}
