...
export function MushafPage({page,width,height,mode,language,playingVerseId,sessionRange,showSession,masked,revealed,onVerseLongPress,onBlankLongPress,onTap}:Props){
  ...
  return <Pressable onPress={onTap} onLongPress={event=>{const {locationX,locationY}=event.nativeEvent;const id=verseAtImagePoint(rows,locationX-2,locationY-2,width-4,height-4);if(id===null)onBlankLongPress();else onVerseLongPress(id);}} delayLongPress={450} style={{width,height,backgroundColor:'white',borderWidth:2,borderColor:colors.beige,borderRadius:9,overflow:'hidden',shadowColor:'#000',shadowOpacity:0.12,shadowRadius:10}}>
    ...
    {rows.filter(row=>{const id=verseId(row[0],row[1]);return id!==null&&verseSelected(id);}).map((row,index)=>{const active=verseId(row[0],row[1])===playingVerseId;return <View key={index} pointerEvents="none" style={{position:'absolute',left:2+row[3]/1920*(width-4),top:2+row[5]/3106*(height-4),width:(row[4]-row[3])/1920*(width-4),height:(row[6]-row[5])/3106*(height-4),backgroundColor:active?'transparent':colors.gold,opacity:active?0.85:0.11,borderWidth:active?2:0,borderColor:colors.green,borderRadius:4}} />;})}
  </Pressable>;
}
