...
export type AudioCommand={serial:number;id:number;action:'listen'|'repeat'|'select'};
type Props={sessionRange:Range;page:number;command?:AudioCommand|null;onVerseChange?:(id:number|null)=>void;fullscreen?:boolean};
type ControlsProps=Props&{dock:Dock;setDock:(value:Dock)=>void};

export function PassageAudioPlayer({sessionRange,page,command,onVerseChange,fullscreen}:Props){
  ...
  return <KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':undefined} style={{backgroundColor:colors.paper,borderTopWidth:dock==='closed'?0:1,borderColor:colors.line,paddingHorizontal:12,paddingBottom:6,maxHeight:dock==='expanded'?'70%':undefined}}>
    {dock==='closed'&&<Button onPress={()=>setDock('expanded')}>▶ Écouter mon passage</Button>}
