...
- Un troisième choix « Moushaf Tajweed » est annoncé dans les réglages mais reste désactivé : les images de pages couleur examinées ont des droits incertains et ne disposent pas de coordonnées de versets vérifiées pour leur propre édition. Voir [TAJWEED_PAGE_SOURCE.md](TAJWEED_PAGE_SOURCE.md).
...
Le lecteur de séance diffuse les fichiers MP3 par verset d’[Al Quran Cloud](https://alquran.cloud/cdn) pour Al-Husary, Alafasy, Al-Minshawi et Maher Al-Muaiqly. [MP3Quran](https://www.mp3quran.net/eng/api) fournit l’audio Hafs d’Abdullah Al-Johany et de Saad Al-Ghamdi par sourate, ainsi que les repères temporels exacts de chaque verset. La synchronisation visuelle du moushaf utilise les coordonnées de `bounds.json`. Les fichiers restent hébergés par les sources ; l’application ne les republie pas. Consulter les [conditions d’Al Quran Cloud](https://alquran.cloud/terms-and-conditions) et celles de MP3Quran avant tout ajout de téléchargement hors ligne ou de distribution audio. Les répétitions sont calculées sur la plage de versets Hafs de la séance. Les repères MP3Quran d’Al-Johany manquent pour les versets 4:166 à 4:176 ; le lecteur indique cette indisponibilité au lieu de jouer une plage approximative.
...
Le workflow **Actions → APK Android autonome → Run workflow** compile aussi un APK installable depuis GitHub, signé avec la clé Android de développement générée par Expo. Le [run 0.2.0 GitHub](https://github.com/Msoumaya2019/coran-memoire/actions/runs/35869745800) a réussi avec la configuration Firebase et les variables Supabase publiques du dépôt. Cette clé de développement sert aux essais ; pour une distribution durable et les mises à jour, utiliser une clé de publication stable via EAS.
...
Le contrôle TypeScript et 34 tests automatisés passent, dont les répétitions 1, 2, 3, 5 et 10 fois, les 6 236 correspondances de Tajweed et de traduction, l'identification exacte d'une zone de verset, l'exclusion des objectifs entièrement connus et la sauvegarde des quatre thèmes. Le parcours depuis An-Nâs couvre les 6 236 versets une seule fois et termine par Al-Fatiha. L'export Metro Android de la version 0.5.0 a été généré avec les 604 pages traditionnelles et les quatre nouveaux bandeaux. Les compilations iOS et Android de cette version et les interactions sur de vrais téléphones restent à vérifier.
